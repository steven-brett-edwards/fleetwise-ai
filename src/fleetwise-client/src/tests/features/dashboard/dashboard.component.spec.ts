...
import { createMockFleetSummary, createMockMaintenanceSchedule, createMockWorkOrder } from '../../helpers/mock-data.factory';
...
describe('DashboardComponent', () => {
  ...
  beforeEach(() => {
    ...
    mockMaintenanceService = jasmine.createSpyObj('MaintenanceService', ['getOverdue', 'getUpcoming']);
  ...
  it('ngOnInit_WhenForkJoinCompletes_SetsOverdueSchedules', () => {
    // Setup
    const expectedBrakeSchedule = createMockMaintenanceSchedule({ id: 1, maintenanceType: 'BrakeInspection' });
    const expectedOilSchedule = createMockMaintenanceSchedule({ id: 2, maintenanceType: 'OilChange' });
    mockVehicleService.getSummary.and.returnValue(of(createMockFleetSummary()));
    mockMaintenanceService.getOverdue.and.returnValue(of([expectedBrakeSchedule, expectedOilSchedule]));
    mockMaintenanceService.getUpcoming.and.returnValue(of([]));
    mockWorkOrderService.getAll.and.returnValue(of([]));
    component = createComponent();
  ...
  it('ngOnInit_WhenForkJoinCompletes_SetsUpcomingSchedules', () => {
    // Setup
    const expectedTireSchedule = createMockMaintenanceSchedule({ maintenanceType: 'TireRotation' });
    mockVehicleService.getSummary.and.returnValue(of(createMockFleetSummary()));
    mockMaintenanceService.getOverdue.and.returnValue(of([]));
  ...
  it('ngOnInit_WhenForkJoinCompletes_SetsOpenWorkOrderCount', () => {
    ...
    mockMaintenanceService.getOverdue.and.returnValue(of([]));
    mockMaintenanceService.getUpcoming.and.returnValue(of([]));
    mockWorkOrderService.getAll.and.returnValue(of([openWorkOrder1, openWorkOrder2, openWorkOrder3]));
    component = createComponent();
